...
<div align="right">
		<a href="/listProduct.do?searchCondition=0&searchKeyword=${search.searchKeyword}">낮은 가격순</a>
		<a href="/listProduct.do?searchCondition=1&searchKeyword=${search.searchKeyword}">높은 가격순</a>
		<a href="/listProduct.do?searchCondition=2&searchKeyword=${search.searchKeyword}">최신순</a>
		<a href="/listProduct.do?searchCondition=3&searchKeyword=${search.searchKeyword}">조회순</a>
	...
	<c:forEach var="vo" items="${list}">
		...
		<c:choose>
			<c:when test="${empty vo.proTranCode }">
				<a href="/updateProductView.do?prodNo=${vo.prodNo}">${vo.prodName}</a>
			</c:when>
			<c:otherwise>
				${vo.prodName}
			</c:otherwise>
		...
		<td align="center">
			...
			<c:if test="${fn:trim(vo.proTranCode) eq '1'}">
				${'구매완료'} 상태입니다.
				<a href="/updateTranCodeByProd.do?prodNo=${vo.prodNo}&tranCode=2&currentPage=${search.currentPage}&searchCondition=${search.searchCondition}&searchKeyword=${search.searchKeyword}">배송시작</a>
			</c:if>
