...
<table width="100%" border="0" cellspacing="0" cellpadding="0"	style="margin-top: 10px;">
	...
	<tr class="ct_list_pop">
		...
		<td align="left">현재
				...
				<%="판매중"%>
				<%}else if((vo.getTranCode().trim()).equals(TransCode.purchaseComplete)) {%>
					<%="구매완료"%>
				<%}else if((vo.getTranCode().trim()).equals(TransCode.onDelivery)) {%>
					<%="배송중" %>
				<%}else if((vo.getTranCode().trim()).equals(TransCode.completeDelivery)) {%>
					<%="배송완료"%>
				<%}%>
				상태 입니다.</td>
		<td></td>
		...
		<%}else if((vo.getTranCode().trim()).equals(TransCode.onDelivery)) {%>
			<%="배송중" %>
			<a href="/updateTranCode.do?tranNo=<%=vo.getTranNo()%>&tranCode=3">물건수령</a>
		<%}else if((vo.getTranCode().trim()).equals(TransCode.completeDelivery)) {%>
			<%="배송완료"%>
		<%}%>
		</td>		
